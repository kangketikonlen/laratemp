...
<div x-data="wysiwygEditor({ value: @js($value), placeholder: @js($placeholder) })" x-init="init()" class="rich-editor">
    ...
    <div id="{{ $id }}_editor" x-ref="editor" contenteditable="true" class="rich-editor__surface"
        :class="{ 'rich-editor__surface--empty': isEmpty }" :data-placeholder="placeholder" @input="sync()"
        @blur="sync()"></div>

    <textarea id="{{ $id }}" name="{{ $name }}" x-model="value" class="hidden"></textarea>
</div>
