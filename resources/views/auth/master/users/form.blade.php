<x-layouts.private-module :title="$title" :description="$description">
    <div class="private-page">
        <x-private.page-header :title="$isEdit ? 'Ubah Pengguna' : 'Tambah Pengguna'" :subtitle="$isEdit ? 'Perbarui data pengguna dan penetapan role.' : 'Tambahkan pengguna baru ke sistem.'">
            <x-slot:actions>
                <a href="{{ route('master.users.index') }}" class="private-action-link icon-action tooltip-trigger"
                    aria-label="Kembali ke pengguna" title="Kembali ke pengguna">
        ...
        <x-private.panel :title="$isEdit ? 'Form Ubah Pengguna' : 'Form Tambah Pengguna'"
            ...
            <form method="POST"
                ...
                <x-form.field for="name" label="Nama Lengkap" :error="$errors->first('name')">
                    <x-form.input id="name" name="name" icon="user" :value="old('name', $user->name)"
                        placeholder="Nama lengkap user" required />
                </x-form.field>

                <x-form.field for="username" label="Username" :error="$errors->first('username')">
                    <x-form.input id="username" name="username" icon="user" :value="old('username', $user->username)"
                ...
                <x-form.field for="email" label="Email" :error="$errors->first('email')" class="private-field-span-2">
                    <x-form.input id="email" name="email" type="email" icon="user" :value="old('email', $user->email)"
                ...
                <x-form.field for="password_confirmation" label="Konfirmasi Password" class="private-field">
                    <x-form.password-input id="password_confirmation" name="password_confirmation"
                        placeholder="Ulangi password" :required="!$isEdit" />
                </x-form.field>
                    ...
                    <div class="private-checkbox-grid">
                        ...
                        @empty
                            <div class="private-panel-empty">
                                Belum ada role yang tersedia. Tambahkan role terlebih dahulu sebelum menetapkan ke
                                pengguna.
                            </div>
                        @endforelse
                    </div>
