<x-layouts.private-module :title="$title" :description="$description">
    ...
    <div class="private-page">
        ...
        <x-private.panel title="Daftar Akses Role"
            description="Kelola akses berdasarkan role agar pengaturan pengguna tetap rapi dan konsisten."
            ...
            <form method="GET" action="{{ route('settings.permissions.index') }}" class="private-toolbar">
                <div class="private-search">
                    <x-form.input name="search" :value="$search" icon="settings"
                        placeholder="Cari role, display name, atau deskripsi..." autocomplete="off" />
                    ...
                    <thead>
                        <tr>
                            <x-private.table-sort-heading :href="$sortUrl('display_name')" label="Role" :active="$sort === 'display_name'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('name')" label="Nama Sistem" :active="$sort === 'name'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('permissions_count')" label="Access" :active="$sort === 'permissions_count'"
                                :direction="$direction" />
        ...
        <x-private.panel title="Daftar Akses Langsung Pengguna"
            description="Gunakan akses langsung per pengguna hanya untuk kebutuhan khusus yang tidak cocok dikelola dari role."
            :badge="$users->total() . ' data'">
            @if ($users->isEmpty())
                <div class="private-panel-empty">
                    ...
                    <tbody>
                        @foreach ($users as $managedUser)
                            <tr>
                                ...
                                <td>
                                    <span class="private-table-muted">{{ number_format($managedUser->roles_count) }}
                                        role</span>
                                </td>
                                <td>
                                    <span
                                        class="private-table-muted">{{ number_format($managedUser->permissions_count) }}
                                        aturan akses langsung</span>
                                </td>
                                <td>
                                    <div class="private-table-actions">
