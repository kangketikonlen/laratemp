<x-layouts.private-module :title="$title" :description="$description">
    <div class="private-page">
        <x-private.page-header title="Detail Aktivitas"
            subtitle="Lihat detail lengkap dari satu log aktivitas yang tercatat otomatis.">
            <x-slot:actions>
                <div class="flex flex-wrap items-center gap-3">
                    @if ($previousLog)
                        <a href="{{ route('report.activity-log.show', $previousLog) }}"
                            class="private-action-link icon-action tooltip-trigger" aria-label="Aktivitas sebelumnya"
                    ...
                    @else
                        <span class="private-action-link private-action-link--disabled icon-action tooltip-trigger"
                            aria-label="Aktivitas sebelumnya tidak tersedia"
                            title="Aktivitas sebelumnya tidak tersedia">
                            <x-ui.icon name="chevron-left" class="h-4 w-4" />
                        </span>
                    @endif

                    @if ($nextLog)
                        <a href="{{ route('report.activity-log.show', $nextLog) }}"
                            class="private-action-link icon-action tooltip-trigger" aria-label="Aktivitas berikutnya"
                            title="Aktivitas berikutnya">
                            <x-ui.icon name="chevron-right" class="h-4 w-4" />
                        </a>
                    @else
                        <span class="private-action-link private-action-link--disabled icon-action tooltip-trigger"
                            aria-label="Aktivitas berikutnya tidak tersedia"
                            title="Aktivitas berikutnya tidak tersedia">
                            <x-ui.icon name="chevron-right" class="h-4 w-4" />
                        </span>
                    @endif

                    <a href="{{ route('report.activity-log.index') }}"
                        class="private-action-link icon-action tooltip-trigger" aria-label="Kembali ke log aktivitas"
                        title="Kembali ke log aktivitas">
        ...
        <x-private.panel title="{{ $log->activity }}"
            ...
            <div class="activity-log-detail-grid">
                ...
                <div class="activity-log-detail-card">
                    ...
                    <span
                        class="private-role-badge private-role-badge--category-{{ $log->category }}">{{ ucfirst($log->category) }}</span>
                </div>

                <div class="activity-log-detail-card">
                    <span class="activity-log-detail-label">Status</span>
                    <span
                        class="private-role-badge private-role-badge--status-{{ $log->status }}">{{ ucfirst($log->status) }}</span>
                </div>
