<x-layouts.private-module :title="$title" :description="$description">
    ...
    <div class="private-page">
        <x-private.page-header title="Linimasa Aktivitas"
            subtitle="Tinjau histori aktivitas penting dari sistem, pengguna, dan operasional tim.">
            <x-slot:actions>
        ...
        <x-private.panel title="Data Aktivitas"
            ...
            <form method="GET" action="{{ route('report.activity-log.index') }}" class="private-toolbar">
                <div class="private-search">
                    <x-form.input name="search" :value="$search" icon="clipboard"
                        placeholder="Cari aktivitas, pelaku, kategori, status, atau detail..." autocomplete="off" />
            ...
            @if ($logs->isEmpty())
                <div class="private-panel-empty">
                    Belum ada log aktivitas yang cocok dengan filter saat ini. Aktivitas akan muncul otomatis setelah
                    pengguna mulai menggunakan aplikasi.
                </div>
            @else
                <x-private.data-table>
                    <thead>
                        <tr>
                            <x-private.table-sort-heading :href="$sortUrl('activity')" label="Aktivitas" :active="$sort === 'activity'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('actor')" label="Pelaku" :active="$sort === 'actor'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('category')" label="Kategori" :active="$sort === 'category'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('status')" label="Status" :active="$sort === 'status'"
                                :direction="$direction" />
                            <x-private.table-sort-heading :href="$sortUrl('logged_at')" label="Waktu Dicatat" :active="$sort === 'logged_at'"
                                :direction="$direction" />
                    ...
                    <tbody>
                        @foreach ($logs as $entry)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="private-role-badge private-role-badge--category-{{ $entry->category }}">{{ ucfirst($entry->category) }}</span>
                                </td>
                                <td>
                                    <span
                                        class="private-role-badge private-role-badge--status-{{ $entry->status }}">{{ ucfirst($entry->status) }}</span>
                                </td>
                                <td>
                                    <span
                                        class="private-table-muted">{{ $entry->logged_at?->format('d M Y H:i') }}</span>
                                </td>
                                <td>
